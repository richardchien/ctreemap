...
void __map_insert_entry(void *_self, void *_entry) {
    BaseMap *self = _self;
...
void *__map_find_entry(void *_self, const char *key) {
    BaseMap *self = _self;
    BaseEntry entry;
...
void __map_remove(void *_self, const char *key) {
    BaseMap *self = _self;
    BaseEntry entry;
    entry.key = strdup(key);
...
void __map_clear(void *_self) {
    BaseMap *self = _self;

    entry_count = 0;
    __map_traverse(self, count_entry_traverse_callback);

    entry_array = malloc(entry_count * sizeof(void *));
    entry_array_cursor = 0;
    __map_traverse(self, collect_entry_traverse_callback);
...
void __map_traverse(void *_self, void (*_traverse_callback)(const void *entry)) {
    BaseMap *self = _self;
    traverse_callback = _traverse_callback;
    twalk(self->root, traverse_action);
}

void __map_destroy(void **pself) {
    BaseMap *self = *pself;
    __map_clear(self);
    free(self->root);
    free(self);
